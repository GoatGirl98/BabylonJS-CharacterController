var CharacterController = /** @class */ (function () {
     ...
     * @param faceForward
     */
    function CharacterController(avatar, camera, scene, actionMap, faceForward, emitter, logit = false) {
    ...
    /**
     * DO PUB
     * run all Pubs from this class through one method to keep them documented in one place 
     * @param {string} topic, the pub topic all listed below
     * @param {object} data, data sent to the subscribers of this topic
     *    topic: CharacterController-doMove
     *    - data: anim_id, anim_group_name, char_pos, char_rot
     *    - def: when a player moves, emits these values related to movement
     *    - subs: handleMovement in playerSync.js
     * 
     *    topic: CharacterController-doJump
     *    - data: anim_id, anim_group_name, char_pos, char_rot
     *    - def: when a player jumps, emits these values related to the jump
     *    - subs: handleMovement in playerSync.js
     */
    CharacterController.prototype.doPub = function(topic, data) {
    }

    //SUBS
    CharacterController.prototype.doSubs = function() {

    }

    //SET NAME
    CharacterController.prototype.setName = function(name) {
    ...
    //DO POINT 
    CharacterController.prototype.doPoint = function(pickResult) {
        for (const property in this.agMap) {
            //console.log(this.agMap[property])
            if (property != 'point'){
                this.agMap[property].pause()
        ...
        this.agMap['point'].play(true);

        if (pickResult != undefined){
            //change avatar rotation to face the picked mesh
            this.mlog.log('CharacterController-doPoint-onPointerDown', {'pickResult.pickedPoint':pickResult.pickedPoint}, 6);
            //change avatar rotation to face mouse position
            var lookAt = new BABYLON.Vector3(pickResult.pickedPoint.x, this._avatar.position.y, pickResult.pickedPoint.z);
            this._avatar.lookAt(lookAt);
            //now flip the avatar 180 degrees
            this._avatar.rotation.y = this._avatar.rotation.y + Math.PI;
        }
        let actData = this._actionMap.point;
        //this.pubAnimData(actData, 'doPoint');
    }
    CharacterController.prototype.stopPoint = function() {
        this.mlog.log('CharacterController-stopPoint', {}, 6);
        this.agMap['point'].stop();
        this.agMap['idle'].play(true);
    }
    
    //DO SwingRight 
    CharacterController.prototype.doSwingRight = function() {
        for (const property in this.agMap) {
            //console.log(this.agMap[property])
            if (property != 'swingRight'){
                this.agMap[property].pause()
            }
        }
        //this.pauseAnim();
        this._swingCount++;
        this.mlog.log('CharacterController-swingRight', {'this._swingCount':this._swingCount}, 6);
        this.agMap['swingRight'].play(true);
        //in increments of 5, increase the speed of the swing
        if (this._swingCount % 5 == 0){
            this.agMap['swingRight'].speedRatio += 0.1;
        }
        let actData = this._actionMap.swingRight;
        //this.pubAnimData(actData, 'swingRight');
    }
    CharacterController.prototype.stopSwing = function() {
        this.mlog.log('CharacterController-stopSwing', {'this._swingCount':this._swingCount}, 6);
        this.agMap['swingRight'].stop();
        this._swingCount = 0;
        this.agMap['swingRight'].speedRatio = 1;
    }
    
    

    //sends the animation data to doPub above, only if the player is player1
    CharacterController.prototype.pubAnimData = function(anim, eventName) {
        this.mlog.log('CharacterController-pubAnimData', {'this._playerName':this._playerName, anim:anim, eventName:eventName}, 6);
        if ((anim != null) && (this._playerName == 'player1')){
            let animGroupName = '';
            if (anim.hasOwnProperty("ag")){
                animGroupName = anim.ag.name;
            }
            let emitval = {
                anim_id: anim.id, 
                anim_group_name: animGroupName,
                char_pos: this._avatar.position,
                char_rot: this._avatar.rotation
            }
            this.doPub('CharacterController-'+eventName, emitval);
        }
    }
    ...
    CharacterController.prototype.setDeathAnim = function (rangeName, rate, loop) {
        this._setAnim(this._actionMap.death, rangeName, rate, loop);
    };
    
    CharacterController.prototype.setPointAnim = function (rangeName, rate, loop) {
        this._setAnim(this._actionMap.point, rangeName, rate, loop);
    };
    
    
    CharacterController.prototype.setSwingRightAnim = function (rangeName, rate, loop) {
        this._setAnim(this._actionMap.swingRight, rangeName, rate, loop);
    };
    ...
    /*Defaults are
        ...
        " "	jump
    */
    CharacterController.prototype.setDeathKey = function (key) {
        this._actionMap.death.key = key.toLowerCase();
    };
    
    CharacterController.prototype.setPointKey = function (key) {
        this._actionMap.point.key = key.toLowerCase();
    };
    CharacterController.prototype.setSwingRightKey = function (key) {
        this._actionMap.swingRight.key = key.toLowerCase();
    };
    ...
    CharacterController.prototype._doJump = function (dt) {
        ...
        //console.log({disp})
        //Use CC's re-positioning method only for player1 - player2 is controlled by direct repositioning from synced move data
        if (this._playerName == 'player1') {
        ...
        //emit the data about this jump
        //this.pubAnimData(actData, 'doJump');
    ...
    CharacterController.prototype._doMove = function (dt) {
        ...
        //emit event to be picked up by playerSync to sync this players location
        //this event is received in playerSync.js
        //emit the data about this move
        //this.pubAnimData(actdata, 'doMove');
    ...
    CharacterController.prototype._doIdle = function (dt) {
        ...
        if (this._freeFallDist < 0.01)
            return anim;
        var disp = new BABYLON.Vector3(0, -this._freeFallDist, 0);
        if (this._hasCam && this._mode != 1 && !this._noRot)
            this._avatar.rotation.y = this._av2cam - this._camera.alpha;
        this._avatar.moveWithCollisions(disp);
    ...
    CharacterController.prototype._onKeyDown = function (e) {
        ...
        //this.doPub('CharacterController-Key', {'_onKeyDown':e});
        this._move = this.anyMovement();
    };
    CharacterController.prototype._onKeyUp = function (e) {
        this.mlog.log('CharacterController-_onKeyUp', {'this._playerName':this._playerName}, 6);
        ...
        //this.doPub('CharacterController-Key', {'_onKeyUp':e});
        this._move = this.anyMovement();
    };
    ...
    CharacterController.prototype.fastrun = function (b) {
        this._act._walk = b;
        this._act._speedMod = b;
    };

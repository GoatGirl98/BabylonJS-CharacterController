...
function loadPlayer(scene, engine, canvas) {
    BABYLON.SceneLoader.ImportMesh("", "player/", "bear.glb", scene, (meshes, particleSystems, skeletons, animationGroups) => {
        var player = meshes[0];
        ...
        */
        ...
        for (let i = 0; i < animationGroups.length; ++i)
            agMap[animationGroups[i].name] = animationGroups[i], animationGroups[i].stop(), console.log(i + "," + animationGroups[i].name);


        //var agMap = createAGmap(allAGs);

        cc = new CharacterController(player, camera, scene, agMap, true, null);

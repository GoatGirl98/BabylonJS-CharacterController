...
export class CharacterController {
    ...
    public start() {
        ...
        if (this._pointerLock) this.openPinterlock();
    ...
    public stop() {
        ...
        if (!this._pointerLock) this.removePinterlockEvent();
        this._prevActData = null;
    }
    ...
    private _updateTargetValue() {
        ...
        if (this._camera.radius <= this._camera.lowerRadiusLimit) {
            ...
            if (this._inFP && this._noFirstPerson) {
    ...
    // Pointerlock 
    private _pointerlockChangeHandler = this.onPointerlockChange.bind(this)
    private onPinterlock() {
        let canvas: HTMLCanvasElement = <HTMLCanvasElement>this._scene.getEngine().getRenderingCanvas();
    ...
    private onPinterunlock(e: MouseEvent) {
        if (e.button === 2) {
            document.exitPointerLock()
        }
    }
    private removePinterlockEvent() {
        let canvas: HTMLCanvasElement = <HTMLCanvasElement>this._scene.getEngine().getRenderingCanvas();
        canvas.removeEventListener("click", this.onPinterlock, false)
        canvas.removeEventListener("mouseup", this.onPinterlock, false)
        document.removeEventListener('pointerlockchange', this._pointerlockChangeHandler, false)
        document.removeEventListener('mozpointerlockchange', this._pointerlockChangeHandler, false)
        document.removeEventListener('webkitpointerlockchange', this._pointerlockChangeHandler, false)
    }
    public setPointerlock(lock: boolean) {
        this._pointerLock = lock
    }
    private openPinterlock() {
        let canvas: HTMLCanvasElement = <HTMLCanvasElement>this._scene.getEngine().getRenderingCanvas();
        // register the callback when a pointerlock event occurs
        document.addEventListener('pointerlockchange', this._pointerlockChangeHandler, false);
        document.addEventListener('mozpointerlockchange', this._pointerlockChangeHandler, false);
        document.addEventListener('webkitpointerlockchange', this._pointerlockChangeHandler, false);
        document.exitPointerLock = document.exitPointerLock ||
            document.mozExitPointerLock ||
            document.webkitExitPointerLock;
        // let that = this
        canvas.addEventListener("click", this.onPinterlock.bind(this), false)
        canvas.addEventListener("contextmenu", (e) => {
            e.preventDefault()
        })
        canvas.addEventListener("mouseup", this.onPinterunlock.bind(this), false)
    }
